...
export default async function CommunityPostPage({ params, searchParams }: { params: Promise<{ postId: string }>; searchParams: Promise<{ category?: string; action?: string }> }) {
  ...
  if (supabase) {
    ...
    const { data: post } = await supabase.from("community_posts").select("id,author_id,post_type,title,body,region_city,region_suburb,created_at,view_count,score,share_count").eq("id", postId).eq("status", "published").maybeSingle();
    if (post) {
      ...
      const [{ count: responseCount }, { data: vote }] = await Promise.all([
