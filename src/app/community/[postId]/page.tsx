...
export default async function CommunityPostPage({ params, searchParams }: { params: Promise<{ postId: string }>; searchParams: Promise<{ category?: string }> }) {
  ...
  return <CommunityPostDetailClient post={{ id: fallback.id, type: fallback.type, title: fallback.title, body: cleanHtml(fallback.excerpt), location: fallback.location, createdAt: fallback.timeAgo ?? fallback.eventDate ?? "Recently", authorName: null, authorAvatarUrl: null, viewCount: fallback.viewCount ?? 0, isOwner: false, images: fallback.image ? [{ src: fallback.image, alt: fallback.imageAlt ?? fallback.title }] : [] }} relatedCategory={relatedCategory} />;
}
