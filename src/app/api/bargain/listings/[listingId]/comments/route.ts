...
export async function GET(_: Request, { params }: { params: Promise<{ listingId: string }> }) {
  ...
  const [{ data: profileData }, { data: voteData }] = await Promise.all([
    authorIds.length
      ? supabase.from("bargain_comment_profiles").select("id,display_name,avatar_path").in("id", authorIds)
      : Promise.resolve({ data: [] }),
    user && comments.length
      ? supabase.from("bargain_listing_comment_votes").select("comment_id,value").eq("user_id", user.id).in("comment_id", comments.map((comment) => comment.id))
      : Promise.resolve({ data: [] }),
  ]);
...
export async function POST(request: Request, { params }: { params: Promise<{ listingId: string }> }) {
  ...
  const rateLimit = await consumeMarketRateLimit(supabase, "comment");
  if (!rateLimit.allowed) {
    return rateLimit.reason === "unavailable"
      ? NextResponse.json({ error: "Unable to post your comment right now. Please try again in a moment." }, { status: 503 })
      : NextResponse.json({ error: "You are posting comments too quickly. Please try again in a minute." }, { status: 429 });
  }
