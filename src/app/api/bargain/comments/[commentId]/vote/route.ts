...
export async function PUT(request: Request, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  const { error } = value === 0
    ? await supabase.from("bargain_listing_comment_votes").delete().eq("comment_id", commentId).eq("user_id", user.id)
    : await supabase.from("bargain_listing_comment_votes").upsert({ comment_id: commentId, user_id: user.id, value, updated_at: new Date().toISOString() }, { onConflict: "comment_id,user_id" });
