...
export async function PATCH(request: Request, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  const { error } = await result.supabase.from("bargain_listing_comments").update({ body, updated_at: new Date().toISOString() }).eq("id", commentId).eq("author_id", result.user.id);
...
export async function DELETE(_: Request, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  const { error } = await result.supabase.from("bargain_listing_comments").update({ body: "Comment deleted", deleted_at: now, updated_at: now }).eq("id", commentId).eq("author_id", result.user.id);
