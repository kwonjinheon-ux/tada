...
export async function POST(request: Request) {
  ...
  const rateLimit = await consumeMarketRateLimit(supabase, "block");
  if (!rateLimit.allowed) {
    return rateLimit.reason === "unavailable"
      ? apiFailure("UNAVAILABLE", "Unable to block this member right now. Please try again in a moment.", 503)
      : apiFailure("RATE_LIMITED", "Too many block requests. Please try again later.", 429);
  }
