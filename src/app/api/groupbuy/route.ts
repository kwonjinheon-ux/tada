...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("group_buys")
    .select("id,title,summary,status,closes_at,handover_at,pickup_available,delivery_available,delivery_fee_cents,group_buy_items(id,name,note,price_cents,unit_label,limit_per_person,photo_alt)")
    .order("created_at", { ascending: false });
  if (error) return apiFailure("INTERNAL", "Unable to load group buys.", 500);
  return apiSuccess((data ?? []).filter((row) => row.group_buy_items.length > 0).map(toGroupBuyCard));
}
...
export async function POST(request: Request) {
  ...
  if (data.items.some((item) => item.photoPath && !item.photoPath.startsWith(`${user.id}/group-buy/`))) {
  ...
  const { data: groupBuy, error } = await supabase.from("group_buys").insert({
    owner_id: user.id, title: data.title, summary: data.summary, description: data.description, reference_prefix: data.referencePrefix,
