...
export default async function EditBargainSaleItemPage({ params }: { params: Promise<{ listingId: string; itemId: string }> }) {
  ...
  if (!user) redirect(`/login?redirectTo=${encodeURIComponent(`/market/${listingId}/items/${itemId}/edit`)}`);
  ...
  const { data: reservations } = await supabase.from("bargain_item_reservations").select("id,amount_cents,status,pickup_start_at,pickup_end_at").eq("listing_id", listingId).eq("item_id", itemId).eq("seller_id", user.id).in("status", ["requested", "confirmed", "on_the_way"]);
  return <BargainSaleItemEditClient listingId={listingId} item={{ id: item.id, title: item.title, description: item.description, priceCents: item.price_cents, imageUrl, storagePath: photo?.storage_path ?? null }} pickupRequests={(reservations ?? []).map((reservation) => ({ id: reservation.id, amountCents: reservation.amount_cents, status: reservation.status as "requested" | "confirmed" | "on_the_way", pickupStartAt: reservation.pickup_start_at, pickupEndAt: reservation.pickup_end_at }))} />;
}
