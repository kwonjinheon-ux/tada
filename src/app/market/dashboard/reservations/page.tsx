...
export default async function BargainReservationsPage() {
  ...
  const renderRows = (rows: ReservationRow[], role: "buying" | "selling") => rows.map((reservation) => {
    ...
    const href = role === "selling" && reservation.status === "pending" ? `/bargain/${reservation.listing_id}/items/${reservation.item_id}/edit` : `/bargain/${reservation.listing_id}`;
