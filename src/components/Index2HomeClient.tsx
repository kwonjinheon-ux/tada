  ...
  return <>
    <main className="index2-home">
      <section className="index2-hero">
        <Image src="/images/home/tada-local-life-hero.png" alt="" fill priority sizes="100vw" className="index2-hero-image" />
      ...
      <PageContainer className="index2-content">
        <section className="index2-nearby" aria-labelledby="index2-nearby-title">
          ...
          <div className="index2-product-rail">{visibleListings.length ? visibleListings.map((listing, index) => <ProductCard key={listing.id} listing={listing} initialIsSaved={savedListingIds.includes(listing.id)} priority={index < 2} imageSizes="(max-width: 767px) 72vw, (max-width: 1279px) 30vw, 220px" />) : <Index2EmptyState />}</div>
        </section>
        ...
        <div className="index2-split-grid">
          ...
          <section className="index2-service-panel ui-card" aria-labelledby="index2-service-title"><Image src="/images/home/journey-services.png" alt="" fill sizes="(max-width: 767px) 100vw, 50vw" /><div><span className="index2-icon"><i className="ms ms-build" aria-hidden="true" /></span><h2 id="index2-service-title">{isKorean ? "우리 동네 서비스" : "Local businesses"}</h2><p>{isKorean ? "믿을 수 있는 지역 서비스를 찾아보세요." : "Support local. Keep your community thriving."}</p><Link href="/services">{isKorean ? "서비스 둘러보기" : "Explore services"} <i className="ms ms-arrow-forward" aria-hidden="true" /></Link></div></section>
        </div>
