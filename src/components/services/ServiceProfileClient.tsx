...
import { serviceBadgeLabel, serviceDetailFields, services, servicesCategoryLabels, type ServiceBadge, type ServiceCategoryId } from "@/data/services";
...
export function ServiceProfileClient({ serviceId }: { serviceId: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const { data: reviewRows, count: exactReviewCount, error: reviewsError } = await supabase.from("service_reviews").select("id,reviewer_id,rating,comment,created_at", { count: "exact" }).eq("service_id", serviceId).order("created_at", { ascending: false });
      if (isCurrent && !reviewsError) {
        const rows = reviewRows ?? [];
        const reviewerIds = [...new Set(rows.map((review) => review.reviewer_id))];
        const { data: reviewerProfiles } = reviewerIds.length ? await supabase.from("profiles").select("id,display_name").in("id", reviewerIds) : { data: [] };
        const names = new Map((reviewerProfiles ?? []).map((reviewer) => [reviewer.id, reviewer.display_name]));
        const nextReviews = rows.map((review) => ({ id: review.id, reviewerId: review.reviewer_id, rating: review.rating, comment: review.comment, createdAt: review.created_at, reviewerName: names.get(review.reviewer_id) || (isKorean ? "Tada 이용자" : "Tada member") }));
        const reviewCount = exactReviewCount ?? nextReviews.length;
        const rating = reviewCount ? nextReviews.reduce((total, review) => total + review.rating, 0) / reviewCount : 0;
        setReviews(nextReviews);
        setProfile((current) => current ? { ...current, reviewCount, rating } : current);
      }
      setIsLoading(false);
    })();
  ...
  const servicePricingRows = serviceOptionRecords.map((service) => serviceFields.map((field) => {
    const rawValue = service[field.key];
    if (typeof rawValue !== "string" || !rawValue) return "—";
    return field.options?.find((option) => option.value === rawValue)?.label ?? rawValue;
  }));
  const priceFromIndex = serviceFields.findIndex((field) => field.key === "price_from");
  const priceUnitIndex = serviceFields.findIndex((field) => field.key === "price_unit");
  const priceLabel = priceFromIndex < 0 || servicePricingRows[0]?.[priceFromIndex] === "—" ? null : `$${servicePricingRows[0][priceFromIndex]}${priceUnitIndex >= 0 && servicePricingRows[0]?.[priceUnitIndex] !== "—" ? ` · ${servicePricingRows[0][priceUnitIndex]}` : ""}`;
  ...
  const hasServiceDetails = servicePricingRows.some((row) => row.some((value) => value !== "—"));
      ...
      <section className="market-results services-results service-profile-main">
        <div className={`service-profile-shell is-section-${activeSection}`} onClick={(event) => {
          ...
          <section className="service-profile-content">
            <article className="service-profile-section service-profile-about ui-card"><header><p className="service-profile-eyebrow">{isKorean ? "서비스 소개" : "About"}</p></header><p>{profile.description}</p>{businessContactDetails.length || businessProfileDetails.length || openingHours.length ? <section className="service-profile-business-details" aria-label={isKorean ? "업체 정보" : "Business information"}><h3>{isKorean ? "업체 정보" : "Business information"}</h3><div className="service-profile-business-columns"><dl>{businessContactDetails.map(({ icon, label, value }) => <div key={label}><i className={`ms ${icon}`} aria-hidden="true" /><div><dt>{label}</dt><dd>{value}</dd></div></div>)}</dl><dl>{businessProfileDetails.map(({ icon, label, value }) => <div key={label}><i className={`ms ${icon}`} aria-hidden="true" /><div><dt>{label}</dt><dd>{value}</dd></div></div>)}{openingHours.length ? <div><i className="ms ms-schedule" aria-hidden="true" /><div><dt>{isKorean ? "운영 시간" : "Opening hours"}</dt><dd><ul className="service-profile-hours">{openingHours.map(({ days, time }, index) => <li key={`${days}-${index}`}><span>{days}</span><span className={/closed|휴무/i.test(time) ? "is-closed" : undefined}>{time}</span></li>)}</ul></dd></div></div> : null}</dl></div></section> : null}</article>
            ...
            {hasGallery ? <article className="service-profile-section service-profile-gallery-section ui-card"><header><p className="service-profile-eyebrow">{isKorean ? "작업 사진" : "Work gallery"}</p></header><ImageGallery images={profile.images.map((src, index) => ({ src, alt: profile.provider + " " + (index + 1) }))} /></article> : null}
            ...
            <article className="service-profile-section service-profile-reviews ui-card"><header className="service-profile-reviews-heading"><p className="service-profile-eyebrow">{isKorean ? "지역 이용자 후기" : "What locals say"}</p><button className="service-review-open" type="button" onClick={() => setIsReviewOpen(true)}><i className="ms ms-star" aria-hidden="true" /> {isKorean ? "후기 등록하기" : "Write a review"}</button></header><div className="service-profile-reviews-layout"><section className="service-profile-review-overview" aria-label={isKorean ? "후기 요약" : "Review summary"}><strong>{profile.rating.toFixed(1)}</strong><ServiceRatingStars rating={profile.rating} label={`${profile.rating.toFixed(1)} / 5`} /><small>{profile.reviewCount} {isKorean ? "개의 후기" : profile.reviewCount === 1 ? "review" : "reviews"}</small></section><section className="service-profile-review-distribution" aria-label={isKorean ? "별점 분포" : "Rating distribution"}>{reviewDistribution.map(({ rating, count }) => <div key={rating}><span>{rating} <i className="ms ms-star" aria-hidden="true" /></span><span><i style={{ width: `${(count / reviewDistributionPeak) * 100}%` }} /></span><small>{count}</small></div>)}</section><section className="service-profile-review-list" aria-label={isKorean ? "최근 후기" : "Recent reviews"}>{reviews.slice(0, 2).map((review) => <article key={review.id}><span className="service-profile-review-avatar" aria-hidden="true">{review.reviewerName.slice(0, 1).toUpperCase()}</span><div><header><strong>{review.reviewerName}</strong><ServiceRatingStars rating={review.rating} label={`${review.rating} / 5`} /><time dateTime={review.createdAt}>{reviewAgeLabel(review.createdAt, isKorean)}</time></header><p>{review.comment}</p></div></article>)}{reviews.length === 0 ? <p className="service-profile-review-empty">{isKorean ? "아직 작성된 후기가 없습니다. 첫 후기를 남겨 주세요." : "No written reviews yet. Be the first to share your experience."}</p> : null}</section></div>{profile.reviewCount > 2 ? <button className="service-profile-review-all" type="button">{isKorean ? `후기 ${profile.reviewCount}개 모두 보기` : `View all ${profile.reviewCount} reviews`}</button> : null}{isReviewOpen ? <ServiceReviewDialog inline serviceId={profile.id} providerName={profile.provider} isKorean={isKorean} onClose={() => setIsReviewOpen(false)} onSubmitted={(review) => { setReviews((current) => [{ ...review, reviewerName: isKorean ? "나" : "You" }, ...current]); setProfile((current) => current ? { ...current, rating: ((current.rating * current.reviewCount) + review.rating) / (current.reviewCount + 1), reviewCount: current.reviewCount + 1 } : current); setIsReviewOpen(false); }} /> : null}</article>
          </section>
        </div>
      </section>
