    ...
    <div className="post-ad-layout">
      <section className="post-ad-card">
        ...
        <form className="post-ad-form" onSubmit={submit} onInvalidCapture={() => setNotice(isKorean ? "필수 정보를 모두 입력해 주세요." : "Complete all required fields to continue.")}>
          <section className="post-title-field"><div className="post-section-heading"><span>1</span><h2>{copy.category}</h2></div><div className="post-shop-type-options" role="group" aria-label={copy.category}>{serviceCategories.map(({ id, icon }) => <button className={category === id ? "is-selected" : ""} key={id} type="button" onClick={() => { setCategory(id); setServiceDetailValues({}); }}><i className={`fa-solid ${icon}`} aria-hidden="true" />{categoryLabels[id]}</button>)}</div></section>
          ...
          <section className="service-trust-requirements"><div className="post-section-heading"><span>6</span><h2>{copy.trustTitle}</h2></div><p>{copy.trustDescription}</p><div>{copy.trust.map(([, title, body], index) => <article key={title}><i className={["fa-solid fa-briefcase", "fa-solid fa-address-card", "fa-solid fa-shield-halved", "fa-solid fa-clipboard-check"][index]} aria-hidden="true" /><span><strong>{title}</strong><small>{body}</small></span></article>)}</div><aside><i className="fa-solid fa-triangle-exclamation" aria-hidden="true" /> {copy.trustAlert}</aside></section>
