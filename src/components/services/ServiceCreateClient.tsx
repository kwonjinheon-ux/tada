...
export function ServiceCreateClient() {
  ...
  return <main className="marketplace-page services-page service-create-page">
    ...
    <div className="post-ad-create-bar"><Link href="/services"><i className="ti ti-arrow-left" aria-hidden="true" /> {copy.back}</Link></div>
    <div className="post-ad-layout">
      <section className="post-ad-card">
        ...
        <section className="service-create-information" aria-label="Service listing information">{copy.information.map(([, title, body], index) => <article key={title}><i className={["ti ti-credit-card", "ti ti-shield-half", "ti ti-circle-check"][index]} aria-hidden="true" /><div><strong>{title}</strong><span>{body}</span></div></article>)}</section>
        <form className="post-ad-form" onSubmit={submit} onInvalidCapture={() => setNotice(isKorean ? "필수 정보를 모두 입력해 주세요." : "Complete all required fields to continue.")}>
          <section className="post-title-field"><div className="post-section-heading"><span>1</span><h2>{copy.category}</h2></div><div className="post-shop-type-options" role="group" aria-label={copy.category}>{serviceCategories.map(({ id, icon }) => <button className={category === id ? "is-selected" : ""} key={id} type="button" onClick={() => { setCategory(id); setServiceDetailValues({}); }}><i className={`ti ${icon}`} aria-hidden="true" />{categoryLabels[id]}</button>)}</div></section>
          ...
          <section className="service-trust-requirements"><div className="post-section-heading"><span>6</span><h2>{copy.trustTitle}</h2></div><p>{copy.trustDescription}</p><div>{copy.trust.map(([, title, body], index) => <article key={title}><i className={["ti ti-briefcase", "ti ti-id", "ti ti-shield-half", "ti ti-clipboard-check"][index]} aria-hidden="true" /><span><strong>{title}</strong><small>{body}</small></span></article>)}</div><aside><i className="ti ti-alert-triangle" aria-hidden="true" /> {copy.trustAlert}</aside></section>
      ...
      <aside className="post-ad-sidebar service-create-sidebar service-create-support-rail" aria-label={isKorean ? "서비스 등록 도움말" : "Service registration help"}><section className="post-ad-tips"><h2>{copy.tipsTitle}</h2>{copy.tips.map((tip, index) => <article key={tip}><i className={["ti ti-list-check", "ti ti-map-pin", "ti ti-shield-half"][index]} aria-hidden="true" /><p>{tip}</p></article>)}</section><section className="service-verification-card"><header><h2>{copy.verifiedTitle}</h2><i className="ti ti-shield-half" aria-hidden="true" /></header><p>{copy.verifiedIntro}</p><ol>{copy.verifiedSteps.map(([number, title, body], index) => <li key={title}><span>{number}</span><i className={["ti ti-file-text", "ti ti-id", "ti ti-circle-check"][index]} aria-hidden="true" /><div><strong>{title}</strong><small>{body}</small></div></li>)}</ol><Link href="/market/dashboard/profile">{copy.verificationAction}</Link><footer><i className="ti ti-circle" aria-hidden="true" /> {copy.verificationStatus}</footer></section></aside>
  </main>;
}
