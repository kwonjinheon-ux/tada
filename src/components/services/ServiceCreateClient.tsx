...
function MultiSelectDropdown({ label, options, selected, onChange, helper, placeholder }: { label: string; options: MultiSelectOption[]; selected: string[]; onChange: (values: string[]) => void; helper: string; placeholder: string }) {
  ...
  return <div className="post-field service-multi-select"><label>{label}</label><details><summary><span>{summary}</span><i className="fa-solid fa-chevron-down" aria-hidden="true" /></summary><div>{options.map((option) => <label key={option.value}><input type="checkbox" checked={selected.includes(option.value)} onChange={(event) => onChange(event.target.checked ? [...selected, option.value] : selected.filter((value) => value !== option.value))} />{option.label}</label>)}</div></details><small>{helper}</small></div>;
}
...
export function ServiceCreateClient() {
  ...
  const allSuburbsValue = "__all_suburbs__";
  const availableSuburbs = serviceAreas.includes(allAreasValue)
    ? [...new Set(NZ_MAIN_LOCATIONS.flatMap((location) => getSubLocations(location)))]
    : [...new Set(serviceAreas.flatMap((location) => NZ_MAIN_LOCATIONS.includes(location as (typeof NZ_MAIN_LOCATIONS)[number]) ? getSubLocations(location as (typeof NZ_MAIN_LOCATIONS)[number]) : []))];
  const selectAreas = (values: string[]) => { const next = values.includes(allAreasValue) ? [allAreasValue] : values; setServiceAreas(next); setSuburbs([]); };
  const selectSuburbs = (values: string[]) => setSuburbs(values.includes(allSuburbsValue) ? [allSuburbsValue] : values);
