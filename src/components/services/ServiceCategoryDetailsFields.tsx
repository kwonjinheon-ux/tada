  ...
  return (
    <div className="service-category-details" aria-live="polite">
      <div className={`service-price-table service-price-table--${fields.length}-fields ${category === "computerIt" ? "service-price-table--computer-it" : ""}`}>
        <div className="service-price-table-head">{fields.map((field) => <span key={field.key}><span className="service-editor-required-mark" aria-hidden="true">*</span>{field.label}</span>)}<span>{isKorean ? "삭제" : "Remove"}</span></div>
