    ...
    <div className="service-category-details" aria-live="polite">
      <div className={`service-price-table service-price-table--${fields.length}-fields ${category === "computerIt" ? "service-price-table--computer-it" : ""}`}>
        ...
        {serviceRows.map((row, rowIndex) => <div className="service-price-row" key={row.id}>{fields.map((field) => {
          ...
          return <label key={field.key}><span className="service-editor-field-label">{field.key === "price_from" && rowIndex === 0 ? <span className="service-editor-required-mark">*</span> : null}{field.label}</span><input id={name} name={name} type={field.input} required={rowIndex === 0} min={field.min} step={field.step} inputMode={field.input === "number" ? "decimal" : undefined} placeholder={field.placeholder} value={row.values[field.key] ?? ""} onChange={(event) => updateRowValue(row.id, field.key, event.target.value)} /></label>;
