...
function formatListTime(value: string | null) {
  ...
  if (date.toDateString() === today.toDateString()) return new Intl.DateTimeFormat("en-NZ", { hour: "numeric", minute: "2-digit" }).format(date);
      ...
      <section className="messages-list-panel" aria-label="Conversations">
        ...
        <div className="messages-conversation-list">
          {conversations.length ? conversations.map((conversation) => <button className={`messages-conversation ${conversation.id === selectedConversationId ? "is-active" : ""}`} type="button" key={conversation.id} onClick={() => openConversation(conversation.id)}>
            ...
            <span className="messages-conversation-copy"><span><strong>{conversation.counterpart.name}</strong><time>{formatListTime(conversation.lastMessageAt)}</time></span><em>{conversation.listing.title}</em><small>{conversation.lastMessagePreview || "Start the conversation"}</small></span>
