...
const bargainCategories = [
  { value: "all", label: "All Bargains", icon: "fa-border-all" },
...
export function BargainPageClient({ postedListings = [], savedListingIds = [] }: { postedListings?: Listing[]; savedListingIds?: string[] }) {
  ...
  return <main className="marketplace-page bargain-page market-page-with-bottom-dock">
    ...
    <MobileDrawer open={isFilterOpen} onClose={() => setIsFilterOpen(false)} ariaLabel="Close bargain filters" className="filter-backdrop" panelClassName="market-filter-panel">
      ...
      <section className="filter-block category-filter"><h2>Bargain type</h2><div className="filter-list">{bargainCategories.map((category) => <button key={category.value} className={`${mobileDrawerClasses.menuItem} ${mobileDrawerClasses.staggerItem} ${bargain === category.value ? "is-selected" : ""}`} type="button" onClick={() => chooseBargain(category.value)}><i className={`fa-solid ${category.icon}`} aria-hidden="true" /><span className={mobileDrawerClasses.menuLabel}>{category.label}</span></button>)}</div></section>
    </MobileDrawer>
    <section className="market-results bargain-results" aria-label="Bargain listings">
      <section className="bargain-hero"><div><p>BARGAIN FINDS</p><h1>Bargain</h1><span>Tiny prices, quick finds, local deals</span><ul><li><i className="fa-solid fa-coins" aria-hidden="true" /> Low prices <small>Easy on your wallet</small></li><li><i className="fa-solid fa-bolt" aria-hidden="true" /> Quick finds <small>Grab it before it’s gone</small></li><li><i className="fa-solid fa-location-dot" aria-hidden="true" /> Local deals <small>From people like you</small></li></ul></div><div className="bargain-hero-art" aria-hidden="true"><i className="fa-solid fa-bag-shopping" /><i className="fa-solid fa-tags" /><i className="fa-solid fa-sparkles" /></div></section>
      <div className="bargain-quick-deals" aria-label="Quick bargain categories">{quickDeals.map((deal) => <button key={deal.value} className={bargain === deal.value ? "is-selected" : ""} type="button" onClick={() => chooseBargain(deal.value)}><i className={`fa-solid ${deal.icon}`} aria-hidden="true" /><span><strong>{deal.title}</strong><small>{deal.description}</small></span></button>)}</div>
      <div className="market-toolbar bargain-toolbar"><div className="view-toggle" aria-label="View mode"><button className={viewMode === "list" ? "is-selected" : ""} type="button" aria-label="List view" aria-pressed={viewMode === "list"} onClick={() => setViewMode("list")}><i className="fa-solid fa-list" aria-hidden="true" /></button><button className={viewMode === "grid" ? "is-selected" : ""} type="button" aria-label="Grid view" aria-pressed={viewMode === "grid"} onClick={() => setViewMode("grid")}><i className="fa-solid fa-border-all" aria-hidden="true" /></button></div><p>Showing {listings.length} bargain{listings.length === 1 ? "" : "s"}</p><label className="sort-control" aria-label="Sort bargains"><select value={searchParams.get("sort") ?? "newest"} onChange={(event) => updateParams({ sort: event.target.value === "newest" ? null : event.target.value })}><option value="newest">Newest</option><option value="priceAsc">Low to High</option><option value="priceDesc">High to Low</option></select></label></div>
      {listings.length ? <div className={`product-grid ${viewMode === "list" ? "is-list-view" : ""}`}>{listings.map((listing, index) => <ProductCard key={listing.id} listing={listing} priority={index === 0} initialIsSaved={savedIdSet.has(listing.id)} />)}</div> : <div className="market-search-empty" role="status"><i className="fa-solid fa-tags" aria-hidden="true" /><strong>No bargains found yet</strong><span>Try another deal type or nearby location.</span></div>}
    </section>
  </main>;
}
