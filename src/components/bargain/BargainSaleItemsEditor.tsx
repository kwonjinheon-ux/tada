...
export function BargainSaleItemsEditor({ photos, items, onChange, onAddItem, onRemoveItem, onGenerateDescriptions, isGeneratingDescriptions, generationProgress, generationError }: {
  ...
  if (!photos.length) return <section className="bargain-sale-items bargain-sale-items-empty" aria-label="Itemized inventory"><div><strong>Itemized inventory</strong><p>Add photos to create individual listing slots.</p></div><Button className="bargain-sale-add-item" variant="secondary" size="sm" onClick={onAddItem}><i className="ti ti-plus" aria-hidden="true" /> Add your first item</Button></section>;
  ...
  return <section className="bargain-sale-items" aria-label="Individual sale item details">
    <div className="bargain-sale-items-heading">
      ...
      <Button className="bargain-sale-add-item" variant="secondary" size="sm" onClick={onAddItem}><i className="ti ti-plus" aria-hidden="true" /> Add another item</Button>
      ...
      {photos.map((photo, index) => {
        ...
        return <article className="bargain-sale-item" key={photo.id}>
          ...
          <div className="bargain-sale-item-fields">
            <div className="bargain-sale-item-topline"><strong>Item {index + 1}</strong><button type="button" onClick={() => onRemoveItem(photo.id)} aria-label={`Remove item ${index + 1}`}><i className="ti ti-trash" aria-hidden="true" /> Remove</button></div>
            <div className="bargain-sale-item-meta"><label>Item title<input type="text" value={item.title} onChange={(event) => onChange(photo.id, "title", event.target.value)} placeholder="e.g. Oak dining table" required /></label><SelectMenu id={`item-category-${photo.id}`} name={`item_category_${photo.id}`} label="Category" icon="ti-tags" placeholder="Select category" options={itemCategoryOptions} value={item.category} onChange={(value) => onChange(photo.id, "category", value)} className="bargain-item-category-select" /></div>
