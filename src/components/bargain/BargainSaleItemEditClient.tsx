...
export function BargainSaleItemEditClient({ listingId, item, pickupRequests }: { listingId: string; item: { id: string; title: string; description: string; priceCents: number; imageUrl: string; storagePath: string | null }; pickupRequests: PickupRequest[] }) {
  ...
  const pickupTime = (value: string | null) => value ? new Intl.DateTimeFormat("en-NZ", { weekday: "short", day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }).format(new Date(value)) : "No time selected";
  return <main className="bargain-item-edit-page"><PageContainer><PageInner size="form"><Link className="listing-detail-back" href={`/market/${listingId}`}><i className="fa-solid fa-arrow-left" aria-hidden="true" /> Back to sale</Link><section className="ui-card bargain-item-edit-card"><h1>Edit item</h1><form onSubmit={save}><label>Photo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void choosePhoto(event)} /></label><div className="bargain-item-edit-preview"><Image src={previewUrl} alt="Item preview" fill unoptimized sizes="(max-width: 767px) 100vw, 480px" /></div><label>Item title<input className="ui-input" value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Price (NZD)<input className="ui-input" type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label><label>Description<textarea className="ui-input" value={description} onChange={(event) => setDescription(event.target.value)} /></label>{error ? <p className="bargain-sale-action-error" role="alert">{error}</p> : null}<div className="bargain-item-edit-actions"><Button type="submit" disabled={isSaving}>{isSaving ? "Saving…" : "Save item"}</Button><Button variant="secondary" onClick={() => router.push(`/market/${listingId}`)}>Cancel</Button></div></form></section>{pickupRequests.length ? <section className="ui-card bargain-item-offers"><h2>Pickup commitments</h2><p>Confirming creates a temporary hold until the requested pickup time. Mark it picked up only when the buyer arrives.</p>{pickupRequests.map((request) => <div key={request.id}><span><strong>${(request.amountCents / 100).toFixed(2)}</strong><small>{request.status === "requested" ? `Requested for ${pickupTime(request.pickupStartAt)}` : request.status === "on_the_way" ? "Buyer is on the way" : `Confirmed for ${pickupTime(request.pickupStartAt)}`}</small></span><span>{request.status === "requested" ? <><Button size="sm" onClick={() => void respondToOffer(request.id, "accept")} disabled={isResponding !== null}>{isResponding === request.id ? "Saving…" : "Confirm pickup"}</Button><Button variant="secondary" size="sm" onClick={() => void respondToOffer(request.id, "decline")} disabled={isResponding !== null}>Decline</Button></> : <><Button size="sm" onClick={() => void respondToOffer(request.id, "picked_up")} disabled={isResponding !== null}>{isResponding === request.id ? "Saving…" : "Mark picked up"}</Button><Button variant="secondary" size="sm" onClick={() => void respondToOffer(request.id, "no_show")} disabled={isResponding !== null}>No-show</Button></>}</span></div>)}</section> : null}</PageInner></PageContainer></main>;
}
