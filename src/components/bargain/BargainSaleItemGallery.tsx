...
export function BargainSaleItemGallery({ activeIndex, items, onClose, onSelect }: BargainSaleItemGalleryProps) {
  ...
  return <DialogOverlay className="listing-gallery-lightbox bargain-sale-item-gallery" onClose={onClose} aria-label={`${item.title} photo gallery`}>
    ...
    {items.length > 1 ? <>
      <button className="listing-gallery-lightbox-arrow is-previous" type="button" aria-label="Previous item" onClick={() => showItem(activeIndex - 1, "previous")}><i className="fa-solid fa-chevron-left" aria-hidden="true" /></button>
      <button className="listing-gallery-lightbox-arrow is-next" type="button" aria-label="Next item" onClick={() => showItem(activeIndex + 1, "next")}><i className="fa-solid fa-chevron-right" aria-hidden="true" /></button>
    </> : null}
