...
export function BargainSaleDetailClient({ sale }: { sale: BargainSaleDetail }) {
  ...
  return <main className="bargain-sale-detail-page"><PageContainer>
    <div className="listing-detail-back-row" aria-label={t("bargainNavigation")}><Link className="listing-detail-back" href="/market"><i className="ti ti-arrow-left" aria-hidden="true" /> {t("bargainBackToListings")}</Link><nav className="listing-detail-category-path" aria-label={t("marketType")}><Link href="/market">{t("market")}</Link><span aria-hidden="true">/</span><Link href={typeHref}>{typeLabel}</Link></nav></div>
    <div className="bargain-sale-detail-layout">
      <section className="bargain-sale-detail-overview"><div className="bargain-sale-detail-hero"><Image src={sale.coverImage.src} alt={sale.coverImage.alt} fill priority unoptimized sizes="(max-width: 1200px) 100vw, 780px" /><div className="bargain-sale-detail-hero-shade" /><div className="bargain-sale-detail-hero-content"><span className="bargain-sale-detail-type">{typeLabel}</span><h1>{sale.title}</h1><p>{sale.description}</p></div></div>
        <div className="bargain-sale-summary-cards"><section className="bargain-sale-event-card"><h2><i className="ti ti-calendar-month" aria-hidden="true" /> {t("bargainEventLogistics")}</h2><dl>{sale.dateLabel ? <div><dt><i className="ti ti-calendar" aria-hidden="true" /><span className="sr-only">{t("bargainDate")}</span></dt><dd>{sale.dateLabel}</dd></div> : null}{sale.timeLabel ? <div><dt><i className="ti ti-clock" aria-hidden="true" /><span className="sr-only">{t("bargainTime")}</span></dt><dd>{sale.timeLabel}</dd></div> : null}<div><dt><i className="ti ti-map-pin" aria-hidden="true" /><span className="sr-only">{t("location")}</span></dt><dd>{sale.address ?? sale.location}</dd></div></dl>{sale.address ? <a className="bargain-sale-directions" href={directionsHref} target="_blank" rel="noreferrer">{t("bargainGetDirections")} <i className="ti ti-external-link" aria-hidden="true" /></a> : <p className="bargain-sale-pickup-privacy"><i className="ti ti-lock" aria-hidden="true" /> {t("bargainPickupLocationPrivacy")}</p>}</section>
          <section className="bargain-sale-host-card"><h2><i className="ti ti-user" aria-hidden="true" /> {t("bargainSellerInfo")}</h2><div className="bargain-sale-host-profile"><span className="bargain-sale-host-avatar">{sale.seller.avatarUrl ? <Image src={sale.seller.avatarUrl} alt="" fill unoptimized sizes="56px" /> : <span aria-hidden="true">{sale.seller.name.slice(0, 1).toUpperCase()}</span>}</span><div><strong>{sale.seller.name}</strong><span>{t("bargainLocalSeller")}</span></div></div><div className="bargain-sale-host-actions"><Button variant="secondary" size="sm" block><i className="ti ti-message" aria-hidden="true" /> {t("bargainMessageSeller")}</Button><Button variant="secondary" size="sm" block onClick={() => void shareSale()}><i className={isShareCopied ? "ti ti-check" : "ti ti-share"} aria-hidden="true" /> {isShareCopied ? t("communityCopied") : t("communityShare")}</Button></div></section></div>
      </section>
      <section className="bargain-sale-detail-inventory" aria-labelledby="sale-inventory-title"><header><div><h2 id="sale-inventory-title">{t("bargainSaleInventory")} ({items.length} {t("bargainItemCountSuffix")})</h2><p>{t("bargainInventoryHint")}</p></div><label className="bargain-sale-search"><i className="ti ti-search" aria-hidden="true" /><span className="sr-only">{t("bargainSearchInventoryLabel")}</span><input type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={t("bargainSearchInventoryPlaceholder")} /></label></header><div className="bargain-sale-category-tabs" aria-label={t("bargainFilterItems")}>{categories.map((category) => <button key={category} type="button" className={activeCategory === category ? "is-selected" : ""} onClick={() => setActiveCategory(category)}>{category === "All items" ? t("bargainAllItems") : category}</button>)}</div>
        {visibleItems.length ? <div className="bargain-sale-detail-items">{visibleItems.map((item, index) => <article className={`bargain-sale-detail-item ${item.status === "sold" ? "is-sold" : ""}`} key={item.id}><button className="bargain-sale-detail-item-image" type="button" onClick={() => setGalleryIndex(items.findIndex((saleItem) => saleItem.id === item.id))} aria-label={`${item.title} — ${t("bargainPhotoGallery")}`}><Image src={item.image.src} alt={item.image.alt} fill unoptimized sizes="(max-width: 767px) 100vw, 280px" />{item.status === "sold" ? <span>{t("bargainSold")}</span> : <b>{item.status === "reserved" ? t("bargainHeldForPickup") : t("available")}</b>}</button><div className="bargain-sale-detail-item-copy"><div><h3>{index + 1}. {item.title}</h3><strong>{formatMarketPrice(item.priceCents)}</strong></div><p>{item.description}</p><div className="bargain-sale-item-actions">{sale.viewerIsOwner ? <Link className="ui-button ui-button--primary primary-button ui-button--sm ui-button--block" href={`/market/${sale.id}/items/${item.id}/edit`}>{item.pendingReservationIds.length ? `${t("bargainReviewPickup")} (${item.pendingReservationIds.length})` : t("bargainEditItem")}</Link> : item.viewerReservation ? <><span className={`bargain-sale-pickup-status is-${item.viewerReservation.status}`}>{item.viewerReservation.status === "requested" ? t("bargainPickupRequestedStatus") : item.viewerReservation.status === "on_the_way" ? t("bargainOnYourWay") : `${t("bargainPickupConfirmed")} · ${pickupLabel(item.viewerReservation.pickupStartAt)}`}</span>{item.viewerReservation.status === "confirmed" ? <Button size="sm" block disabled={busyId !== null} onClick={() => void updatePickup(item.viewerReservation!.id, "on_the_way")}>{t("bargainImOnMyWay")}</Button> : null}<Button variant="secondary" size="sm" block disabled={busyId !== null} onClick={() => void updatePickup(item.viewerReservation!.id, "cancel")}>{t("bargainCancelPickup")}</Button></> : <Button size="sm" block disabled={item.status !== "available" || busyId === item.id} onClick={() => openReserveDialog(item.id)}>{busyId === item.id ? t("bargainSending") : item.status === "reserved" ? t("bargainHeldForPickup") : t("bargainPlanVisit")}</Button>}</div></div></article>)}</div> : <div className="bargain-sale-detail-empty" role="status"><i className="ti ti-package" aria-hidden="true" /><strong>{t("bargainNoItemsInCategory")}</strong><span>{t("bargainChooseAnotherCategory")}</span></div>}
      </section>
      <aside className="bargain-sale-detail-sidebar"><a className="bargain-sale-map-card" href={directionsHref} target="_blank" rel="noreferrer" aria-label={`${t("bargainGetDirections")}: ${mapLocation}`}><iframe className="bargain-sale-map-embed" src={mapEmbedSrc} title={`${t("bargainMapLabel")}: ${mapLocation}`} loading="lazy" tabIndex={-1} aria-hidden="true" /><i className="ti ti-map-pin" aria-hidden="true" /><span>{t("bargainOpenInGoogleMaps")}</span></a><section id="event-location" className="bargain-sale-about-card"><h2>{t("bargainAboutSale")}</h2><p>{sale.description}</p><ul><li><i className="ti ti-cash" aria-hidden="true" /> {t("bargainPaymentNote")}</li><li><i className="ti ti-shopping-bag" aria-hidden="true" /> {t("bargainBagsNote")}</li></ul></section></aside>
