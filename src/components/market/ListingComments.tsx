  ...
  const renderComment = (comment: ListingComment) => {
    ...
    return <article className={`listing-comment depth-${comment.depth}`} key={comment.id}>
      ...
      <div className="listing-comment-content">
        ...
        {!comment.deletedAt ? <div className="listing-comment-tools"><button type="button" className={comment.myVote === 1 ? "is-selected" : ""} onClick={() => void vote(comment, 1)} disabled={busyCommentId === comment.id} aria-label="Upvote comment"><i className="fa-solid fa-arrow-up" aria-hidden="true" /> <span>{comment.score}</span></button>{canReply ? <button type="button" className="listing-comment-text-button" onClick={() => { setReplyTo(comment); setReplyDraft(""); expandThread(comment.id); }}>Reply</button> : null}<button type="button" className={comment.myVote === -1 ? "is-selected is-downvote" : ""} onClick={() => void vote(comment, -1)} disabled={busyCommentId === comment.id} aria-label="Downvote comment"><i className="fa-solid fa-arrow-down" aria-hidden="true" /></button>{isOwner ? <><button type="button" className="listing-comment-text-button" onClick={() => { setEditingId(comment.id); setEditDraft(comment.body); }}>Edit</button><button type="button" className="listing-comment-text-button is-danger" onClick={() => void deleteComment(comment)}>Delete</button></> : null}</div> : null}
