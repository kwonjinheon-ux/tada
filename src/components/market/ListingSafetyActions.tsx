...
export function ListingSafetyActions({ listingId, sellerId, sellerProfileVariant = false, iconOnly = false, space = "market" }: { listingId: string; sellerId: string | null; sellerProfileVariant?: boolean; iconOnly?: boolean; space?: "market" | "community" }) {
  ...
  return <section className="listing-safety-actions" aria-label="Listing safety tools">
    <div className={`listing-safety-buttons ${sellerProfileVariant ? "is-seller-profile-variant" : ""}`}>
      <button type="button" aria-label="Report listing" title="Report" onClick={() => setIsReportOpen(true)}><i className="ms ms-flag" aria-hidden="true" /> {!iconOnly && (sellerProfileVariant ? "신고" : "Report")}</button>
      {sellerId ? <button type="button" aria-label="Block seller" title="Block seller" onClick={() => void blockSeller()} disabled={isSubmitting}><i className="ms ms-block" aria-hidden="true" /> {!iconOnly && (sellerProfileVariant ? "차단" : "Block")}</button> : null}
    </div>
    ...
    {isReportOpen ? <DialogOverlay className="listing-report-dialog" aria-labelledby="listing-report-title" onClose={() => setIsReportOpen(false)} isDismissible={!isSubmitting}>
      <div className="listing-report-panel">
        <button className="listing-report-close" type="button" aria-label="Close report form" onClick={() => setIsReportOpen(false)}><i className="ms ms-close" aria-hidden="true" /></button>
