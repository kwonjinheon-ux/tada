    ...
    <div className="dashboard-content profile-settings-content wishlist-content">
      ...
      {visibleItems.length ? <section className="wishlist-list" aria-label="Saved items">
        {visibleItems.map((item) => <article className={`wishlist-item ${item.status === "Sold" ? "is-sold" : ""}`} key={item.id}>
          ...
          <div className="wishlist-item-main"><p>{item.category}</p><h2>{item.title}</h2><div className="wishlist-item-actions"><Link href={`/market/${item.id}`}>View listing</Link><button className="wishlist-secondary-action" type="button" disabled={messagingId === item.id} onClick={() => void openConversation(item.id)}>{messagingId === item.id ? "Opening..." : "Send message"}</button><button className="wishlist-remove-action" type="button" disabled={updatingIds.has(item.id)} onClick={() => void removeItem(item)}><i className="fa-solid fa-xmark" aria-hidden="true" /> Remove</button></div></div>
          <div className="wishlist-item-side"><strong>{item.price}</strong></div>
        </article>)}
