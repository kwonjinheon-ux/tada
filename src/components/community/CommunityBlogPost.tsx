...
export function CommunityBlogPost({ post, showTypeBadge }: { post: CommunityPost; showTypeBadge: boolean }) {
  ...
  return <article className={`community-blog-post community-post-card-${post.type}`}>
    ...
    <div className="community-blog-post-footer"><div><CommunityPostActions postId={post.id} commentCount={count} /><button type="button" className="community-comments-toggle" aria-expanded={commentsOpen} onClick={() => { setHasOpenedComments(true); setCommentsOpen((open) => !open); }}>{commentsOpen ? "Hide comments" : "View comments"}</button></div><div><ListingSafetyActions listingId={post.id} sellerId={null} sellerProfileVariant iconOnly space="community" /><Link href={`/community/${post.id}`}>Open post <i className="fa-solid fa-arrow-right" aria-hidden="true" /></Link></div></div>
