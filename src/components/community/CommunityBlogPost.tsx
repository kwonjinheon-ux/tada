...
export function CommunityBlogPost({ post, showTypeBadge, mutedTypeBadge = false }: { post: CommunityPost; showTypeBadge: boolean; mutedTypeBadge?: boolean }) {
  ...
  return <article ref={articleRef} className={`community-blog-post community-post-card-${post.type}`}>
    ...
    {isGalleryOpen && images[activeImage] ? <DialogOverlay className="listing-gallery-lightbox" aria-label={`${post.title} photo gallery`} onClose={() => setIsGalleryOpen(false)}>
      ...
      {images.length > 1 ? <><button className="listing-gallery-lightbox-arrow is-previous" type="button" aria-label="Previous photo" onClick={() => showImage(activeImage - 1)}><i className="ti ti-chevron-left" aria-hidden="true" /></button><button className="listing-gallery-lightbox-arrow is-next" type="button" aria-label="Next photo" onClick={() => showImage(activeImage + 1)}><i className="ti ti-chevron-right" aria-hidden="true" /></button><span className="listing-gallery-lightbox-count">{activeImage + 1} / {images.length}</span></> : null}
    </DialogOverlay> : null}
  </article>;
}
