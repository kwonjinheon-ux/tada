...
export function CommunityPostActions({ postId, title, body, commentCount = 0, score = 0, myVote = 0, shareCount = 0, onCommentsToggle, commentsOpen = false, hideComments = false }: { postId: string; title?: string; body?: string; commentCount?: number; score?: number; myVote?: -1 | 0 | 1; shareCount?: number; onCommentsToggle?: () => void; commentsOpen?: boolean; hideComments?: boolean }) {
  const { t } = useLanguage();
  ...
  const share = async () => { const url = `${window.location.origin}/community/${postId}`; const summary = [title, body?.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim(), url].filter(Boolean).join("\n\n"); try { await navigator.clipboard.writeText(summary); } catch { if (navigator.share) await navigator.share({ title, text: body?.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim(), url }); } const response = await fetch(`/api/community/posts/${postId}/engagement`, { method: "POST" }); const payload = response.ok ? await response.json() as { shareCount?: number } : null; if (typeof payload?.shareCount === "number") setCurrentShareCount(payload.shareCount); setShared(true); window.setTimeout(() => setShared(false), 2000); };
