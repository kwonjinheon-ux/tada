  ...
  return <main className="listing-detail-page community-detail-page">
    <PageContainer className="community-detail-container">
      ...
      <article className="ui-card community-detail-post">
        ...
        <footer className="community-detail-post-footer"><div className="community-detail-post-actions"><CommunityPostActions postId={post.id} compact />{post.isOwner ? <CommunityPostOwnerMenu onEdit={openEditDialog} onDelete={() => setIsDeleteDialogOpen(true)} /> : null}</div><div><ListingSafetyActions listingId={post.id} sellerId={null} sellerProfileVariant iconOnly space="community" /><Link href="/community">{t("communityMorePosts")} <i className="fa-solid fa-arrow-right" aria-hidden="true" /></Link></div></footer>
      </article>
