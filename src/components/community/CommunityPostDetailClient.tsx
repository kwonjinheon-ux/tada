    ...
    <PageContainer className="community-detail-container">
      ...
      <article className="ui-card community-detail-post">
        <header className="community-detail-post-header">
          ...
          <div className="community-detail-author"><Avatar src={post.authorAvatarUrl} name={post.authorName} className="community-detail-author-avatar" initials="double" /><strong>{post.authorName}</strong>{post.isOwner ? <button className="listing-detail-save listing-detail-delete" type="button" aria-label="Delete post" onClick={() => setIsDeleteDialogOpen(true)}><i className="fa-solid fa-trash-can" aria-hidden="true" /></button> : null}</div>
        </header>
        ...
        <footer className="community-detail-post-footer"><CommunityPostActions postId={post.id} /><div><ListingSafetyActions listingId={post.id} sellerId={null} sellerProfileVariant iconOnly space="community" /><Link href="/community">More community posts <i className="fa-solid fa-arrow-right" aria-hidden="true" /></Link></div></footer>
      </article>
