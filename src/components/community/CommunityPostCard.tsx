  ...
  return (
    <article className={`community-post-card community-post-card-${post.type} ${featuredImage ? "" : "community-post-card-no-media"}`}>
      <div className="community-post-card-link">
        ...
        <Link className="community-post-body-link" href={href ?? `/community/${post.id}`} aria-label={`Open ${post.title}`} onClick={() => { void fetch(`/api/community/posts/${post.id}/view`, { method: "POST", keepalive: true }); }}>
          ...
          <div className="community-post-meta">
            <span className="community-post-vote-summary" aria-label={`${post.score ?? 0} votes`}><i className="ms ms-arrow-upward" aria-hidden="true" />{new Intl.NumberFormat("en-NZ").format(post.score ?? 0)}<i className="ms ms-arrow-downward" aria-hidden="true" /></span>
      ...
      {isGalleryOpen && galleryImage ? <DialogOverlay className="listing-gallery-lightbox" aria-label={`${post.title} photo gallery`} onClose={() => setIsGalleryOpen(false)}>
        ...
        {galleryImages.length > 1 ? <><button className="listing-gallery-lightbox-arrow is-previous" type="button" aria-label="Previous photo" onClick={() => showImage(activeImage - 1)}><i className="ms ms-chevron-left" aria-hidden="true" /></button><button className="listing-gallery-lightbox-arrow is-next" type="button" aria-label="Next photo" onClick={() => showImage(activeImage + 1)}><i className="ms ms-chevron-right" aria-hidden="true" /></button><span className="listing-gallery-lightbox-count">{activeImage + 1} / {galleryImages.length}</span></> : null}
      </DialogOverlay> : null}
    </article>
  );
}
