  ...
  return <>
    <section className="admin-advertising-overview" aria-label="Advertising placement setup">
      ...
      <div className="admin-advertising-placement-grid">{AD_PLACEMENTS.map((placement) => {
        ...
        return <div className="admin-advertising-placement-group" key={placement}><article className={`admin-advertising-placement${isSelected ? " is-selected" : ""}`}><div className="admin-advertising-placement-heading"><i className="ms ms-ad" aria-hidden="true" /><div><h3>{placementLabels[placement]}</h3><span>{ads.filter((ad) => ad.isActive).length} active · {ads.length} total</span></div></div><AdPlacementPreview placement={placement} /><dl><div><dt>Desktop</dt><dd>{guidance.desktop}</dd></div><div><dt>Mobile</dt><dd>{guidance.mobile}</dd></div></dl><Button variant="secondary" className="admin-advertising-placement-action" onClick={() => openCreate(placement)} aria-expanded={isSelected} aria-controls={`advertising-create-form-${placement}`}>{isSelected ? "Creating here" : "Add to placement"}</Button></article><div id={`advertising-create-form-${placement}`} className="admin-advertising-inline-form-mount" /></div>;
      })}</div>
    </section>

    <section className="admin-advertising-campaigns" aria-labelledby="advertising-campaigns-title"><div className="admin-advertising-section-heading"><div><span className="admin-advertising-eyebrow">Campaigns</span><h2 id="advertising-campaigns-title">Your advertising</h2></div><span>{initialAds.length} total</span></div>{initialAds.length ? <div className="admin-advertising-campaign-list">{initialAds.map((ad) => <article key={ad.id}><div className={`admin-advertising-provider ${ad.provider}`}><i className={`ms ${ad.provider === "adsense" ? "ms-show-chart" : "ms-handshake"}`} aria-hidden="true" /></div><div><strong>{ad.name}</strong><span>{placementLabels[ad.placement]} · Priority {ad.priority}</span></div><span className={`admin-advertising-status ${ad.isActive ? "is-active" : ""}`}>{ad.isActive ? "Active" : "Disabled"}</span><button className="admin-advertising-remove" type="button" disabled={isDeleting === ad.id} onClick={() => void deleteAd(ad.id)} aria-label={`Remove ${ad.name}`}>{isDeleting === ad.id ? "Removing…" : <><i className="ms ms-delete" aria-hidden="true" /> Remove</>}</button></article>)}</div> : <div className="admin-advertising-empty"><i className="ms ms-ad" aria-hidden="true" /><h2>No ads yet</h2><p>Create your first ad from a placement above. It will only start showing after its required details are complete.</p><Button onClick={() => openCreate("market_top")}><i className="ms ms-add" aria-hidden="true" /> Create your first ad</Button></div>}</section>
    ...
    {selectedPlacement && typeof document !== "undefined" ? createPortal(
        ...
        <form onSubmit={saveAd}>
          <header><div><span className="admin-advertising-eyebrow">New campaign</span><h2>Create an ad</h2><p>{placementLabels[form.placement]}</p></div><button type="button" className="admin-advertising-close" aria-label="Close create ad dialog" onClick={closeCreate} disabled={isSaving}><i className="ms ms-close" /></button></header>
          <div className="admin-advertising-provider-choice"><button type="button" className={form.provider === "sponsor" ? "is-selected" : ""} onClick={() => setForm((current) => ({ ...current, provider: "sponsor" }))}><i className="ms ms-handshake" aria-hidden="true" /> Sponsor creative <small>Image and destination</small></button><button type="button" className={form.provider === "adsense" ? "is-selected" : ""} onClick={() => setForm((current) => ({ ...current, provider: "adsense" }))}><i className="ms ms-show-chart" aria-hidden="true" /> Google AdSense <small>Client and slot IDs</small></button></div>
