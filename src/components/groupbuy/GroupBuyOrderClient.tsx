...
export function GroupBuyOrderClient({ groupBuy, basket }: { groupBuy: GroupBuy; basket: Record<string, number> }) {
  ...
  const submit = async (form: HTMLFormElement) => { const supabase = createBrowserSupabaseClient(); const { data: { session } } = await supabase?.auth.getSession() ?? { data: { session: null } }; setSubmitting(true); setError(""); const values = new FormData(form); const response = await fetch(`/api/groupbuy/${groupBuy.id}/orders`, { method: "POST", headers: { "Content-Type": "application/json", ...(session?.access_token ? { Authorization: `Bearer ${session.access_token}` } : {}) }, body: JSON.stringify({ name: values.get("name"), email: values.get("email"), phone: values.get("phone"), fulfilment, address: values.get("address") ?? undefined, note: values.get("note") ?? undefined, lines: lines.map(({ item, quantity }) => ({ itemId: item.id, quantity })) }) }); const result = await response.json(); if (!response.ok) { setError(result.error?.message ?? "Unable to submit order."); setSubmitting(false); return; } router.push(`/market/groupbuy/${groupBuy.id}?order=${result.data.reference}`); };
