...
export function SelectMenu({ id, name, label, icon, placeholder, options, value, onChange, className = "", disabled = false }: { id: string; name: string; label: string; icon?: string; placeholder: string; options: readonly SelectMenuOption[]; value: string; onChange: (value: string) => void; className?: string; disabled?: boolean }) {
  ...
  return <div className={`post-field ${className}`}>
    ...
    <div ref={wrapperRef} className={`post-select-wrap is-enhanced ${icon ? "has-leading-icon" : ""} ${isOpen ? "is-open" : ""}`}>
      ...
      <select id={id} name={name} value={value} disabled={disabled} onChange={(event) => selectValue(event.target.value)}><option value="">{placeholder}</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
      <button className="post-select-trigger" type="button" disabled={disabled} aria-haspopup="listbox" aria-expanded={isOpen} aria-controls={`${id}-menu`} onClick={() => setIsOpen((current) => !current)}><span>{selected?.label ?? placeholder}</span></button>
